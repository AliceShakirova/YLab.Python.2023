...
def test_get_all_inst(clear_storage, insert_inst):
    ...
    test_menu, test_submenu, test_dish = insert_inst(Dish, storage=conftest.ADD_TO_CACHE)
    response = client.get(url=func_reverse('get_all_menus_submenus_and_dishes'))
    assert response.status_code == 200
    test_menu.id = test_response_payload[0]['id']
    test_submenu.id = test_response_payload[1]['id']
    test_dish.id = test_response_payload[2]['id']
