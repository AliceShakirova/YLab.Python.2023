...
def test_get_all_dish_empty(clear_storage, insert_inst):
    ...
    test_menu, test_submenu = insert_inst(Submenu, storage=ADD_TO_CACHE)
...
def test_get_target_dish(clear_storage, insert_inst):
    test_menu, test_submenu = insert_inst(Submenu, storage=ADD_TO_CACHE)
    test_dish_request_payload = {'title': 'My dish 1', 'description': 'My dish description 1', 'price': '12.50'}
    test_response_payload = client.post(url=func_reverse('post_dish', menu_id=test_menu.id,
                                                         submenu_id=test_submenu.id), json=test_dish_request_payload)
...
def test_patch_dish(clear_storage, insert_inst):
    ...
    test_menu, test_submenu = insert_inst(Submenu, storage=ADD_TO_CACHE)
    test_dish_request_payload = {'title': 'My dish 1', 'description': 'My dish description 1', 'price': '12.50'}
    test_post_dish = client.post(url=func_reverse('post_dish', menu_id=test_menu.id,
                                                  submenu_id=test_submenu.id), json=test_dish_request_payload)
...
def test_delete_dish(clear_storage, insert_inst):
    ...
    assert response.status_code == 200
    test_dish_cache = DishCache.get_dish(test_dish.json()['id'])
    assert test_dish_cache is None
    test_menu = MenuCache.get_menu(test_menu.id)
    assert test_menu.submenus_count == 1
    assert test_menu.dishes_count == 0
